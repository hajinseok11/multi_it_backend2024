...
class MyStack{
	// 데이터를 저장할 수 있는 int형 배열이 멤버변수로 정의
	// push, pop, top, empty, size메소드를 정의
	// MyStack클래스 내부에서 데이터를 저장하기 위한 공간(스택처럼 활용)
	private int[] mystack;
	private int top_position; // 스택의 top의 위치값
							  // top_position변수의 값이 0이라는 것은 스택이 비어있다는 의미
	
	public void push(int data) {
		mystack[top_position] = data;
		top_position++;
		System.out.println("배열의 갯수 : "+mystack.length);
		System.out.println("top_position : "+mystack.length);
	}
...
public class Baek_10828_Stack {
	public static void main(String[] args) throws IOException {
		// MyStack의 메소드를 호출해서 테스트케이스 데이터를 입력받아 처리했을 때 출력형식처럼 출력되도록 작업하기
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		int count = Integer.parseInt(br.readLine());
		MyStack stack = new MyStack(count);
		
		for (int i = 0; i<count; i++) {
			String line = br.readLine(); // 한 줄 전체 line입력받아 String리턴
