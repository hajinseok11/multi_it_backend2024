...
<body>
	<!--  
	JSTL (JavaServerPage Standard Tag Library) - 자바 표준 태그 라이브러리
		- 기본 자바코드를 태그로 사용할 수 있도록 지원
		- EL과 같이 사용
		- JSTL을 사용하기 위해서 태그라이브러리를 추가(taglib)
		- 접두사 c로 정의하기 때문에 c:XXXX 태그를 이용해서 작업
		- jsp에서 스크립트릿을 최소화 하기 위해서 사용
	-->
	...
	2. age를 체크해서 age가 70이상이면 실버를 출력하세요
	<c:if test = "${param.age>=70 }">
		<h3>silver</h3>
	</c:if>
	
	3. age를 체크해서 age가 10~20 사이면 청소년을 출력하세요
