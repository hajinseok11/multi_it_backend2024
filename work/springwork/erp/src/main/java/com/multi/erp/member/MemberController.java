...
//스프링MVC프레임워크 내부에서 컨트롤러에서 user라는 이름으로 공유된 Model객체에 저장된 어트리뷰트를 찾아서 
...
public class MemberController {
	...
	public String springlogin(MemberDTO loginUserInfo,Model model) {
		...
		MemberDTO user = service.login(loginUserInfo);
		System.out.println("================로그인성공사용자정보:"+user);
		String view="";
		if(user!=null) {
			model.addAttribute("user", user);
			view = user.getMenupath();//memnu-tiles.xml에 등록한 이름의 뷰를 찾아서 뷰를 생성
		}else {
			//로그인실패
			view = "redirect:/emp/login.do";
		}
